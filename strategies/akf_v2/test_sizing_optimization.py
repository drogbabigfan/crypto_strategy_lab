#!/usr/bin/env python3
"""
CAGR vs MDD Trade-off 최적화
사이즈를 키워서 CAGR을 높이되 MDD를 적정 수준으로 유지
"""
...
def main():
    ...
    # 현재 baseline: Hard=5, Risk=2%, Size=[0.1, 2.0]
    # 목표: CAGR 높이면서 MDD 30% 이하 유지

    configs = [
        # Baseline
        (5.0, 0.02, 0.1, 2.0, "Baseline: H=5, R=2%, Max=2"),

        # Risk Target 증가
        (5.0, 0.03, 0.1, 3.0, "R=3%, Max=3"),
        (5.0, 0.04, 0.1, 4.0, "R=4%, Max=4"),
        (5.0, 0.05, 0.1, 5.0, "R=5%, Max=5"),

        # Hard Stop 축소 (더 타이트 → 더 큰 사이즈)
        (4.0, 0.03, 0.1, 3.0, "H=4, R=3%, Max=3"),
        (3.0, 0.03, 0.1, 3.0, "H=3, R=3%, Max=3"),
        (3.0, 0.04, 0.1, 4.0, "H=3, R=4%, Max=4"),

        # 공격적 조합
        (4.0, 0.04, 0.1, 4.0, "H=4, R=4%, Max=4"),
        (3.0, 0.05, 0.1, 5.0, "H=3, R=5%, Max=5"),
    ]
    ...
    # Summary
    print("\n" + "="*150)
    print("SUMMARY: CAGR vs MDD Trade-off")
    print("="*150)
    print(f"{'Config':<30} {'AvgSharpe':<10} {'AvgCAGR':<10} {'AvgMDD':<10} {'CAGR/MDD':<10} {'AvgSize':<10} {'Pos':<6} {'BTC_CAGR':<10} {'BTC_MDD':<10}")
    print("-"*130)
    ...
    for desc, results in all_results.items():
        ...
        print(f"{desc:<30} {avg_sharpe:<10.2f} {avg_cagr*100:<9.1f}% {avg_mdd*100:<9.1f}% "
              f"{avg_cagr_mdd:<10.2f} {avg_size:<10.2f} {positive}/7    {btc['cagr']*100:<9.1f}% {btc['mdd']*100:<9.1f}%")

    # Best by CAGR/MDD with MDD < 35%
    print("\n" + "="*150)
    print("BEST CONFIG (MDD < 35% 제한)")
    print("="*150)
    ...
    if valid_configs:
        best = max(valid_configs, key=lambda x: np.mean([r["cagr"] for r in x[1]]))
        desc, results = best
        avg_cagr = np.mean([r["cagr"] for r in results])
        avg_mdd = np.mean([r["mdd"] for r in results])
        avg_size = np.mean([r["avg_size"] for r in results])
        ...
        print(f"  BTC: CAGR={btc['cagr']*100:.1f}%, MDD={btc['mdd']*100:.1f}%")
