...
def generate_signals_with_sizing(
    ...
    hard_mult, risk_target, conf_lambda, size_min, size_max, warmup
):
    n = len(close)
...
def calculate_cagr(total_pnl: float, n_bars: int, bars_per_year: float = 365 * 4):
    """Calculate CAGR from total PnL and number of bars"""
...
def run_backtest(df: pd.DataFrame, hard_mult: float = 5.0, risk_target: float = 0.02,
                 size_min: float = 0.1, size_max: float = 2.0):
    ...
    # CAGR 계산
    n_bars = len(df_kf) - 210  # warmup 제외
...
def main():
    ...
    # (hard_mult, risk_target, size_min, size_max)
    configs = [
        # Hard Stop 변화 (Risk 2% 고정)
        (3.0, 0.02, 0.1, 2.0, "Hard=3, R=2%"),
        (4.0, 0.02, 0.1, 2.0, "Hard=4, R=2%"),
        (5.0, 0.02, 0.1, 2.0, "Hard=5, R=2%"),
        (6.0, 0.02, 0.1, 2.0, "Hard=6, R=2%"),
        (8.0, 0.02, 0.1, 2.0, "Hard=8, R=2%"),

        # Risk Target 변화 (Hard 5 고정)
        (5.0, 0.01, 0.1, 1.5, "Hard=5, R=1%"),
        (5.0, 0.03, 0.1, 2.5, "Hard=5, R=3%"),
        (5.0, 0.04, 0.1, 3.0, "Hard=5, R=4%"),

        # 조합
        (4.0, 0.03, 0.1, 2.5, "Hard=4, R=3%"),
        (6.0, 0.03, 0.1, 2.5, "Hard=6, R=3%"),
    ]

    all_results = {}

    for hard_mult, risk_target, size_min, size_max, desc in configs:
        print(f"\n{'='*120}")
        print(f"{desc}")
        print("="*120)
        ...
        for symbol, df in data.items():
            metrics = run_backtest(df, hard_mult, risk_target, size_min, size_max)
        ...
        print(f"Average: Sharpe={avg_sharpe:.2f}, CAGR={avg_cagr*100:.1f}%, MDD={avg_mdd*100:.1f}%, "
              f"CAGR/MDD={avg_cagr_mdd:.2f}, Positive={positive}/7")

    # Summary
    print("\n" + "="*140)
    print("SUMMARY: MDD vs CAGR Trade-off")
    print("="*140)
    print(f"{'Config':<20} {'AvgSharpe':<12} {'AvgCAGR':<12} {'AvgMDD':<12} {'CAGR/MDD':<12} {'Positive':<10} {'BTC_CAGR':<12} {'BTC_MDD':<12}")
    print("-"*120)
    ...
    for desc, results in all_results.items():
        ...
        print(f"{desc:<20} {avg_sharpe:<12.2f} {avg_cagr*100:<11.1f}% {avg_mdd*100:<11.1f}% "
              f"{avg_cagr_mdd:<12.2f} {positive}/7       {btc['cagr']*100:<11.1f}% {btc['mdd']*100:<11.1f}%")
    ...
    # Best Sharpe
    best_sharpe = max(all_results.items(), key=lambda x: np.mean([r["sharpe"] for r in x[1]]))
    print(f"Best Sharpe: {best_sharpe[0]} (Sharpe={np.mean([r['sharpe'] for r in best_sharpe[1]]):.2f})")

    # Best CAGR/MDD
    best_cagr_mdd = max(all_results.items(), key=lambda x: np.mean([r["cagr_mdd"] for r in x[1]]))
    print(f"Best CAGR/MDD: {best_cagr_mdd[0]} (CAGR/MDD={np.mean([r['cagr_mdd'] for r in best_cagr_mdd[1]]):.2f})")

    # Best Positive
    best_positive = max(all_results.items(), key=lambda x: sum(1 for r in x[1] if r["pnl"] > 0))
    print(f"Best Positive: {best_positive[0]} (Positive={sum(1 for r in best_positive[1] if r['pnl'] > 0)}/7)")

    # Lowest MDD
    lowest_mdd = min(all_results.items(), key=lambda x: np.mean([r["mdd"] for r in x[1]]))
    print(f"Lowest MDD: {lowest_mdd[0]} (MDD={np.mean([r['mdd'] for r in lowest_mdd[1]])*100:.1f}%)")
